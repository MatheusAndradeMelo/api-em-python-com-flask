...
class UsuarioModel(banco.Model):
    ...
    def send_confirmation_email(self):
        ...
        return post('https://api.mailgun.net/v3/{}/messages'.format(MAILGUN_DOMAIN),
                    ...
                    'html': '<html><p>\
                        Confirme seu cadastro clicando no link a seguir: <a href="{}">CONFIRMAR EMAIL\
                            </p></html>'.format(link)})
    ...
    def find_by_login(cls, login):
         user = cls.query.filter_by(login=login).first() #Select * from usuarios WHERE user_id = user_id
         if user:
             return user
         return None
    ...
    def find_by_email(cls, email):
         user = cls.query.filter_by(email=email).first() #Select * from usuarios WHERE user_id = user_id
         if user:
             return user
         return None
